...
def require_slack_review(command: "BaseApiCall") -> None:
    ...
    the testreport via :meth:`TestReport.set_slack_review`. This gate reads it
    back and re-queries Slack live so the 👍 ack is confirmed to still be
    present at approve/reject time (durable and re-checkable across sessions).
    ...
    """
    review = command.metadata.slack_review
    if review is None:
